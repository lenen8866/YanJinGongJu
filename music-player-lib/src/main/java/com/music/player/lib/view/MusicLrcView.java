...
public class MusicLrcView extends View {
    ...
    public MusicLrcView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (null != attrs) {
            ...
            textTimeColor = Color.parseColor("#FFFF00");
     ...
     */
    private void loadLrcRows(MusicLrcRowParser lrcRowParser, final String audioID, String object, String textLoading) {
        //如果已经绘制了不做任何处理
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //歌词为空
        if (null == mLrcRows || mLrcRows.size() == 0) {
            mPaint.setColor(Color.parseColor("#5677FC"));
            mPaint.setTextSize(textSize);
            mPaint.setTextAlign(Paint.Align.CENTER);
        ...
        int highlightRowY = (int) (height / 2 + 5);
        mPaint.setColor(textLightColor);
        mPaint.setTextSize(textLightSize);
        mPaint.setTextAlign(Paint.Align.CENTER);
        StaticLayout staticLayoutHighlight = new StaticLayout(highlightText, (TextPaint) mPaint, canvas.getWidth(),
                Layout.Alignment.ALIGN_CENTER, 1, 0, false);
//        canvas.drawText(highlightText, rowX, highlightRowY, mPaint);
        canvas.save();
        canvas.translate(0,highlightRowY);
        staticLayoutHighlight.draw(canvas);
        canvas.restore();
        ...
        if (mDisplayMode == DISPLAY_MODE_SEEK) {
            ...
            canvas.drawText(mLrcRows.get(mHignlightRow).getTimeText(), leftOffset - 20, highlightRowY, mPaint);
        }
        ...
        while (rowY > -textSize && rowNum >= 0) {
            ...
            StaticLayout staticLayoutCenter = new StaticLayout(text, (TextPaint) mPaint, canvas.getWidth(),
                    Layout.Alignment.ALIGN_CENTER, 1, 0, false);
//            canvas.drawText(text, rowX, rowY, mPaint);
        ...
        while (rowY < height && rowNum < mLrcRows.size()) {
            String text = mLrcRows.get(rowNum).getContent();
            StaticLayout staticLayoutBottom = new StaticLayout(text, (TextPaint) mPaint, canvas.getWidth(),
                    Layout.Alignment.ALIGN_CENTER, 1, 0, false);
            canvas.save();
            canvas.translate(0, rowY);
            staticLayoutBottom.draw(canvas);
//            canvas.drawText(text, rowX, rowY, mPaint);
            canvas.restore();
            rowY += (textLineHeight + textSize);
            rowNum++;
    ...
    public void seekTo(long currentDurtion) {
        if (mEnable && null != mLrcRows && mLrcRows.size() > 0) {
            ...
            for (int i = 0; i < mLrcRows.size(); i++) {
                ...
                if ((currentDurtion >= current.getTime() &&
                        next != null && currentDurtion < next.getTime()) ||
