...
public class MusicPlayerService extends Service implements MusicPlayerPresenter, IMediaPlayer.OnPreparedListener,
    ...
    //播放对象监听
    private static List<MusicPlayerInfoListener> sMusicPlayerInfoListener = new ArrayList<>();
    //音频焦点Manager
    ...
    //随机播放 未播放的音频
    private static List<Object> randomAudio = new ArrayList<>();
    //书籍对应全部音乐 全作者
    private ArrayList<?> audioMap = new ArrayList<>();
    //全部书籍
    ...
    public void addPlayMusicToTop(BaseAudioInfo audioInfo) {
        ...
        if (null != playerMusic && playerMusic.id == audioInfo.id) {
            return;
        }
        if (mAudios.size() > 0) {
            int pisition = -1;
...
//                    pause();
                    XToast.showToast(this, "正在加载中，请稍后...");
                    break;
    ...
    public void continuePlay(String sourcePath) {
        ...
        if (null != mAudios && mAudios.size() > mCurrentPlayIndex) {
            ((BaseAudioInfo) mAudios.get(mCurrentPlayIndex)).audio_url = sourcePath;
            startPlayMusic(mCurrentPlayIndex);
        }
    }
    ...
    public void continuePlay(String sourcePath, int index) {
        ...
        if (null != mAudios && mAudios.size() > index) {
            ((BaseAudioInfo) mAudios.get(index)).audio_url = sourcePath;
            startPlayMusic(index);
        }
    }
    ...
    public int setPlayerModel(int model) {
        ...
        if (model == MusicConstants.MUSIC_MODEL_SINGLE) {
            ...
            //设置随机模式，添加随机播放列表
            randomAudio.clear();
            randomAudio.addAll(mAudios);
            if (currentPlayerMusic != null) {
                //设置随机模式 从随机列表中删除该音频
                randomAudio.remove(currentPlayerMusic);
            }
...
//                    break;
        ...
        Log.d(TAG, "playLastMusic--newPlayIndex:" + mCurrentPlayIndex + ",MODE:" + getPlayerModel());
    }
...
//                    break;
        ...
        Log.d(TAG, "playNextMusic--newPlayIndex:" + mCurrentPlayIndex + ",MODE:" + getPlayerModel());
    }
    ...
    public int playLastIndex() {
        ...
        if (null != mAudios && mAudios.size() > 0) {
            switch (getPlayerModel()) {
                ...
                //随机
                case MusicConstants.MUSIC_MODEL_RANDOM:
                    int index = MusicUtils.getInstance().getRandomNum(0, randomAudio.size() - 1);
                    tempIndex = index;
                    break;
            }
        }
        Log.d(TAG, "playLastIndex--LAST_INDEX:" + tempIndex + ",MODE:" + getPlayerModel() + ",CURRENT_INDEX：" + mCurrentPlayIndex);
        return tempIndex;
    }
...
//                    || mMusicPlayerState == MusicConstants.MUSIC_PLAYER_BUFFER);
            ...
            return null != mMediaPlayer && (mMusicPlayerState == MusicConstants.MUSIC_PLAYER_PLAYING
                    || mMusicPlayerState == MusicConstants.MUSIC_PLAYER_BUFFER);
        } catch (RuntimeException e) {
    ...
    public long getCurrentPlayerID() {
        ...
        if (null != mAudios && mAudios.size() > mCurrentPlayIndex) {
            return ((BaseAudioInfo) mAudios.get(mCurrentPlayIndex)).id;
        }
        return 0;
    }
    ...
    public String getCurrentPlayerHashKey() {
        ...
        if (null != mAudios && mAudios.size() > mCurrentPlayIndex) {
            return ((BaseAudioInfo) mAudios.get(mCurrentPlayIndex)).audioHashKey;
        }
        return "";
    }
            ...
            if (null != mOnPlayerEventListeners) {
                ...
                try {
                    for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                        ...
                        if (null != mMediaPlayer || mMusicPlayerState == MusicConstants.MUSIC_PLAYER_PAUSE || isPlaying()) {
    ...
    public void addPlayInfoListener(MusicPlayerInfoListener listener) {
        if (null != sMusicPlayerInfoListener) {
            if (!sMusicPlayerInfoListener.contains(listener)) {
                sMusicPlayerInfoListener.add(listener);
            }
        }
    }
    ...
    public void removePlayInfoListener(MusicPlayerInfoListener listener) {
        if (null != sMusicPlayerInfoListener) {
            sMusicPlayerInfoListener.remove(listener);
        }
    }
    ...
    public void setSpeed(float speed) {
        ...
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                MediaPlayer internalMediaPlayer = mMediaPlayer.getInternalMediaPlayer();
                PlaybackParams playbackParams = internalMediaPlayer.getPlaybackParams();
                playbackParams.setSpeed(speed);
                mMediaPlayer.getInternalMediaPlayer().setPlaybackParams(playbackParams);
            }
        } catch (Exception e) {
        }
    }
    ...
    public void removeAllPlayInfoListener() {
        if (sMusicPlayerInfoListener != null) {
            sMusicPlayerInfoListener.clear();
        }
    }

    private Object audioCateData;
    private int currentCateIndex;

    @Override
    public void setAllCate(Object audioCateData) {
        this.audioCateData = audioCateData;
    }

    @Override
    public Object getAllCate() {
        return audioCateData;
    }

    @Override
    public void setCateIndex(int currentCateIndex) {
        this.currentCateIndex = currentCateIndex;
    }

    @Override
    public int getCateIndex() {
        return currentCateIndex;
    }
    ...
    public void setPlayIndex(int i) {
        if (i < 0) {
            i = 0;
        } else if (i >= getBookAllAudio().size()) {
            i = getBookAllAudio().size() - 1;
        }
        ...
        public void run() {
            try {
                ...
                if (null != mOnPlayerEventListeners) {
                    for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                        if (null != mMediaPlayer && mMediaPlayer.isPlaying()) {
                            long currentDuration = mMediaPlayer.getCurrentPosition() + 500;
                            BaseAudioInfo playerMusic = getCurrentPlayerMusic();
                            if (playerMusic != null) {
                                playerMusic.playDuration = currentDuration + "";
                            }
                            onPlayerEventListener.onTaskRuntime(mMediaPlayer.getDuration(), currentDuration, TIMER_DURTION, mBufferProgress);
                        } else {
    ...
    private synchronized void startPlay(BaseAudioInfo musicInfo) {
        onReset();
        if (musicInfo == null || TextUtils.isEmpty(musicInfo.audio_url)) {
            EventBus.getDefault().post("audio_chapter_no_cache");
            EventBus.getDefault().post("play_stop");
            Toast.makeText(this, "对不起，链接无效", Toast.LENGTH_SHORT).show();
            return;
        }
        startTimer();
        if (null == mAudioFocusManager) {
        ...
        if (requestAudioFocus == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
            ...
            try {
                ...
                musicStatus.setId(musicInfo.id);
                String frontPath = MusicUtils.getInstance().getMusicFrontPath(musicInfo);
                musicStatus.setCover(frontPath);
                musicStatus.setTitle(musicInfo.chapter);
                musicStatus.setPlayerStatus(MusicStatus.PLAYER_STATUS_PREPARED);
                MusicPlayerManager.getInstance().observerUpdata(musicStatus);
                String audioPath = SaltUtils.getUrl(musicInfo.audio_url);
                if (TextUtils.isEmpty(audioPath)) {
                    MusicPlayerService.this.mMusicPlayerState = MusicConstants.MUSIC_PLAYER_ERROR;
                    if (null != mOnPlayerEventListeners) {
                        for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                            onPlayerEventListener.onMusicPlayerState(mMusicPlayerState, "播放失败，地址错误");
                        }
                    }
                    MusicPlayerManager.getInstance().observerUpdata(new MusicStatus(MusicStatus.PLAYER_STATUS_STOP, musicInfo.id));
                    showNotification();
                    return;
                }
                String cachePath = MusicACache.getDiskCachePath(this);
                File file = new File(cachePath, String.valueOf(musicInfo.id));
                if (file.exists()) {
                    audioPath = file.getAbsolutePath();
                } else {
                    if (!isCheckNetwork()) {
                        EventBus.getDefault().post("audio_chapter_no_cache");
                        XToast.showToast(MusicPlayerService.this, "章无缓存,请链接网络");
                        onReset();
                        return;
                    }
                }
                ...
                setSpeed(playSpeed);
                if (null != sMusicPlayerInfoListener) {
                    for (MusicPlayerInfoListener listener : sMusicPlayerInfoListener) {
                        listener.onPlayMusiconInfo(musicInfo, mCurrentPlayIndex);
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
            ...
            switch (getPlayerModel()) {
                ...
                case MusicConstants.MUSIC_MODEL_ORDER:
                    if (mAudios.size() - 1 > mCurrentPlayIndex) {
                        ...
                        for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                            onPlayerEventListener.onMusicPlayerState(MusicConstants.MUSIC_PLAYER_COMPLETE, "列表播放完毕");
                        }
                    }
                    break;
                //随机
                case MusicConstants.MUSIC_MODEL_RANDOM:
                    if (randomAudio.isEmpty()) {
                        //随机列表播完哇完毕
                        for (MusicPlayerEventListener onPlayerEventListener : mOnPlayerEventListeners) {
                            onPlayerEventListener.onMusicPlayerState(MusicConstants.MUSIC_PLAYER_COMPLETE, "列表播放完毕");
                        }
                    ...
                    //随机播放索引
                    int randomIndex = MusicUtils.getInstance().getRandomNum(0, randomAudio.size() - 1);
                    BaseAudioInfo baseAudioInfo = (BaseAudioInfo) randomAudio.get(randomIndex);
                    //获取到真实索引
                    mCurrentPlayIndex = mAudios.indexOf(baseAudioInfo);
                    ...
                    randomAudio.remove(randomIndex);
                    postViewHandlerCurrentPosition(mCurrentPlayIndex);
                    startPlayMusic(mCurrentPlayIndex);
                    break;
            }
     ...
     */
    @Override
    public void onBufferingUpdate(IMediaPlayer mediaPlayer, int progress) {
     ...
     */
    @Override
    public boolean onError(IMediaPlayer mediaPlayer, int event, int extra) {
        MusicPlayerService.this.mMusicPlayerState = MusicConstants.MUSIC_PLAYER_ERROR;
        onReset();
...
//        }
     ...
     */
    @Override
    public boolean onInfo(IMediaPlayer mediaPlayer, int event, int extra) {
        int state = -1;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(AudioManager.ACTION_AUDIO_BECOMING_NOISY)) {
                ...
                if (!TextUtils.isEmpty(mLockActivityClass)) {
                    int playerState = getPlayerState();
                    if (playerState == MusicConstants.MUSIC_PLAYER_PREPARE || playerState == MusicConstants.MUSIC_PLAYER_PLAYING) {
                ...
                if (audioID > 0) {
                    if (!TextUtils.isEmpty(mPlayerActivityClass)) {
                        ...
                        if (appRunning) {
                            ...
                            if (mMediaPlayer != null) {
                                int currentPosition = (int) (mMediaPlayer.getCurrentPosition() + 500);
                                startIntent.putExtra("PLAY_DURATION", currentPosition);
                                int duration = (int) mMediaPlayer.getDuration();
                                startIntent.putExtra("TOTAL_DURATION", duration);
                            }
    ...
    private void showNotification() {
        if (mNotificationEnable) {
            ...
            if (isOpen) {
                ...
                if (null != audioInfo) {
                    //先准备好歌曲封面Bitmap
                    if (audioInfo.image.startsWith("http:") || audioInfo.image.startsWith("https:")) {
                        Picasso.get().load(audioInfo.image).into(new Target() {
                            @Override
                            ...
                            public void onBitmapFailed(Exception e, Drawable errorDrawable) {
                                Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.icon_play_deault_bg);
                                Notification notification = buildNotifyInstance(getCurrentPlayerMusic(), bitmap);
                                showNotification(notification, NOTIFICATION_ID, mForegroundEnable);
                            }
                        ...
                        //File
                        Bitmap bitmap;
                        bitmap = MusicImageCache.getInstance().getBitmap(audioInfo.image);
                        //缓存为空，获取音频文件自身封面
                        //封面为空，使用默认
                        if (null == bitmap) {
                            bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.icon_play_deault_bg);
                        }
                        Notification notification = buildNotifyInstance(getCurrentPlayerMusic(), bitmap);
                        showNotification(notification, NOTIFICATION_ID, mForegroundEnable);
                    }
    ...
    private Notification buildNotifyInstance(BaseAudioInfo audioInfo, Bitmap cover) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            NotificationChannel channel = new NotificationChannel(MusicConstants.CHANNEL_ID, name, NotificationManager.IMPORTANCE_LOW);
            channel.enableVibration(false);
            getNotificationManager().createNotificationChannel(channel);
        }
        //通知栏根部点击意图
        Intent rootIntent = new Intent(MusicConstants.MUSIC_INTENT_ACTION_ROOT_VIEW);
        rootIntent.putExtra(MusicConstants.MUSIC_KEY_MEDIA_ID, audioInfo.id);
        PendingIntent pendClickIntent = PendingIntent.getBroadcast(this, 1, rootIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    private RemoteViews getDefaultCoustomRemoteView(BaseAudioInfo audioInfo, Bitmap cover) {
        ...
        defaultremoteviews.setTextViewText(R.id.music_notice_top_title, audioInfo.chapter);
        defaultremoteviews.setTextViewText(R.id.music_notice_def_title, audioInfo.cate3_name + "-" + audioInfo.author);
        defaultremoteviews.setTextViewText(R.id.music_notice_def_subtitle, audioInfo.cate1_name + "-" + audioInfo.cate2_name);
        //上一首
...
//        bigRemoteViews.setImageViewResource(R.id.music_notice_def_btn_collect, getCollectIcon(audioInfo.getAudioId()));
        bigRemoteViews.setTextViewText(R.id.music_notice_top_title, audioInfo.chapter);
        bigRemoteViews.setTextViewText(R.id.music_notice_def_title, audioInfo.cate3_name + "-" + audioInfo.author);
        bigRemoteViews.setTextViewText(R.id.music_notice_def_subtitle, audioInfo.cate1_name + "-" + audioInfo.cate2_name);
        //上一首
