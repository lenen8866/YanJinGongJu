package com.read.scriptures.view.lrc;
...
public class LrcView extends View {
    ...
    private static final String DEFAULT_CONTENT = "加载中...";

    public ArrayList<Lrc> getLrcData() {
        return mLrcData;
    }

    private ArrayList<Lrc> mLrcData;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mPlayDrawable = mPlayDrawable == null ? ContextCompat.getDrawable(context, R.drawable.icon_play_lrc) : mPlayDrawable;
...
//        ViewConfiguration.getScrollFriction();  默认摩擦力 0.015f
    ...
    public void setLrcData(ArrayList<Lrc> lrcData) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < getLrcCount(); i++) {
            ...
            if (mCurrentLine == i) {
                mTextPaint.setTextSize(mLrcTextSize + 6);
    ...
    private int getUpdateTimeLinePosition(long time) {
        ...
        for (int i = 0; i < getLrcCount(); i++) {
            ...
            if (time >= lrc.getTime()) {//判断时间是否满足要求
                if (i == getLrcCount() - 1) {//判断是否最后一行字幕
                    linePos = getLrcCount() - 1;
                } else if (time < mLrcData.get(i + 1).getTime()) {//判断时间小于下一行字幕的的时间
                    linePos = i;
                    break;
                }
            }
