...
public class BottomSheetBehavior2<V extends View> extends CoordinatorLayout.Behavior<V> {
    public interface  IBottomSheetBehavior2Listener{
        void sheetBehaviorStop(int top);
        void isTouch(boolean isTrue,float downY);
        void initTop( int defaultTop);
    }
    private Context mContext;
    private IBottomSheetBehavior2Listener mIBottomSheetBehavior2Listener;

    public void setmIBottomSheetBehavior2Listener(IBottomSheetBehavior2Listener mIBottomSheetBehavior2Listener) {
        this.mIBottomSheetBehavior2Listener = mIBottomSheetBehavior2Listener;
    }
    ...
    CustomerViewDragHelper mViewDragHelper;
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (mState == STATE_EXPANDED) {
            ViewCompat.offsetTopAndBottom(child, mMinOffset + peekHeight /3);
        ...
        if (mViewDragHelper == null) {
            mViewDragHelper = CustomerViewDragHelper.create(parent, mDragCallback);
        }
...
//        }
    ...
    private void reset() {
        mActivePointerId = CustomerViewDragHelper.INVALID_POINTER;
        if (mVelocityTracker != null) {
    ...
    private final CustomerViewDragHelper.Callback mDragCallback = new CustomerViewDragHelper.Callback() {
        ...
        public void onViewDragStateChanged(int state) {
            if (state == CustomerViewDragHelper.STATE_DRAGGING) {
                setStateInternal(STATE_DRAGGING);
            }
        }
...
//                targetState = STATE_COLLAPSED;
//            }
            if (mIBottomSheetBehavior2Listener != null){
                mIBottomSheetBehavior2Listener.initTop(releasedChild.getTop());
            }
            Log.d("BottomSheetBehavior","yvel===="+yvel);
            targetState=STATE_SETTLING;
            if( Math.abs(yvel) > 700){//重新处理
            ...
            Log.d("BottomSheetBehavior","top===="+top);
            if (mContext != null && top > mPeekHeight){
                top =  mPeekHeight - (int) DisplayUtil.dp2px(mContext,3);
            }
            if (mViewDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top)) {
                ...
                setStateInternal(targetState);
                if (mIBottomSheetBehavior2Listener != null){
                    mIBottomSheetBehavior2Listener.sheetBehaviorStop(top);
                }
            }
        }
        ...
        @Override
        public void isTouch(boolean isTouch,float downY) {
            super.isTouch(isTouch,downY);
            if (mIBottomSheetBehavior2Listener != null){
                mIBottomSheetBehavior2Listener.isTouch(isTouch,downY);
            }
        }
    };
