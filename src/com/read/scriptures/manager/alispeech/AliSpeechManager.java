package com.read.scriptures.manager.alispeech;
...
public class AliSpeechManager {
    ...
    private AliSpeechManager.SwitchThread mSwitchThread;
    private AliSpeechManager.PlayTimeChangeListener playTimeChangeListener;
    ...
    private boolean isExecuteFinish = true;

    private String currentStartTaskId="";
    private String currentEndTaskId="";
    ...
    public AliSpeechManager(Context context, Handler mainHandler) {
        mContext = context;
        ...
        mAudioTrack = new AudioPlayer(new AudioPlayerCallback() {
            ...
            public void playOver() {
                ...
                if (!mIsPause) {
                    ...
                    AliSpeechManager.this.mainHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            AliSpeechManager.this.mainHandler.sendMessage(AliSpeechManager.this.mainHandler.obtainMessage(UI_FINISH_TEXT_SELECTION, 0, 0));
...
//        }
        ...
        //开始播放
        int result = NativeNui.GetInstance().startTts("1", "", mSpeechContext);
        if (result == TTS_TEXT_ERROR) {
            mainHandler.sendMessage(mainHandler.obtainMessage(UI_FINISH_TEXT_SELECTION, 0, 0));
        }
        Log.i(TAG, "开始:" + result);
    ...
    public void resetSpeaking() {
        ...
        ThreadUtil.doOnOtherThread(new Runnable() {
            ...
            public void run() {
                ...
                mAudioTrack.isFinishSend(false);
                mAudioTrack.stop();
                int result = NativeNui.GetInstance().cancelTts("");
    ...
    public void resume() {
        ...
        if (mIsPause) {
            ...
            ThreadUtil.doOnOtherThread(new Runnable() {
                @Override
                public void run() {
                    int result = NativeNui.GetInstance().resumeTts();
                    mAudioTrack.play();
                    Log.i(TAG, "resume：" + result);
                    isExecuteFinish = true;
                }
            });
        }
    ...
    public void startAutoFlowTimer(int time) {
        ...
        mSwitchThread = new AliSpeechManager.SwitchThread();
        mSwitchThread.start();
        isStopThread = false;
    }
    ...
    public void setPlayTimeChangeListener(AliSpeechManager.PlayTimeChangeListener playTimeChangeListener) {
